// * 해당 코드는 '어떤 역할을 해낼까요?'
// 먼저 bundleFiles라는 '이해하기 어려워 보이는' 함수가 큰 부분을 차지하고 있습니다. 유효성 검사를 하기 위해 dataTypeChecker라는 모듈화 되어있는 함수를 불러내었습니다. bundleFiles 함수는 options라는 매개변수가 지정되면 지정된 객체의 파일 경로를 불러냅니다. 그 후 cssFile 변수와 jsFile변수를 파일 읽기 내부 함수와 HTML내 태그 교체 함수를 활용, 새롭게 재편성하는 역할을 한다고 이해했습니다.
// * 능력을 확인했다면, '어떤 절차'로 처리되었을까요?
// 마치 아이가 가지고 노는 레고 블럭처럼 하나씩 조립하며 맞춰가는 형태라고 생각이 듭니다. 하나의 완성작이 아닌 작은 조각으로 끼워 맞추게 되면 자동차가 되었던 레고 블록이 근사한 성으로 재조립되어지듯이 필요한 블록만 쏙쏙 가져가는 것과 같이 캡슐화된 객체를 원하는 입맛에 따라 편집할 수 있습니다. 이는 모듈화된 함수를 호출하는 것과 같습니다. 뼈가 있으면 뼈에 근육이 붙고 근육에는 살이 붙는것처럼 함수를 차곡차곡 쌓아올려내는 비슷한 절차를 처리되지 않았나 짐작해봅니다.
